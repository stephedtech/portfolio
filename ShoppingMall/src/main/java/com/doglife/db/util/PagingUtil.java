...
public class PagingUtil {
	...
	public String makePaging() {
		...
		//1. 전체 페이지 개수 구하기(페이지당 listCnt(10개)만큼 출력)
		//전체 게시글이 9개: 1 페이지
		//전체 게시글이 11개: 2 페이지
		...
		//이전버튼 
		if(startNum != 1) {
			sb.append("<a class='pno' href='./" + listName + "pageNum=" + (startNum-1)+ "'>");
			sb.append("&nbsp;이전&nbsp;</a>");
		}//<a class = 'pno' href='./list?pageNum=5'> 이전 </a>
		//페이지 번호를 5개씩 보여주는 경우
		//6페이지가 보이는 화면에서 이전 버튼이 보이게 되고 그 이전 버튼의 링크는 5페이지가 된다
		...
		//다음 버튼 처리
		if(endNum != totalPage) {
			sb.append("<a class='pno' href='./" + listName + "pageNum=" + (endNum + 1)+ "'>");
			sb.append("&nbsp;다음&nbsp;</a>");
